...
export class TenantsService {
  private readonly apiBase = 'https://localhost:7145/api/Tenant';

  constructor(private readonly http: HttpClient) {}

  getAll(payload?: any): Observable<any[]> {
    const body = payload ?? {};
    return this.http.post<any>(`${this.apiBase}/GetAll`, body).pipe(
      map((res: any) => {
  ...
  create(payload: any): Observable<any> {
    return this.http.post<any>(`${this.apiBase}/Create`, payload);
  }

  delete(id: string | number): Observable<any> {
    const body = { id } as any;
    return this.http.post<any>(`${this.apiBase}/Delete`, body);
  }
}
